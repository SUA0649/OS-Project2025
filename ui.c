...
int init_ui(){
	...
	//Initiallize screen
	initscr();
	//less stricter version of raw() (which would enter the program without the need to press enter and without processing special inputs like ctrl + c for signalin)
	//cbreak() allows for singaling keys
	cbreak();		
	
	//raw();
	//arrow keys no more work
	keypad(stdscr, FALSE);	
	
	//Don't display the message you type, unless explicitly mention like it has in sending message function
	noecho();
